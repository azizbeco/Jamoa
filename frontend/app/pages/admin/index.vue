...
<template>
  <div class="space-y-12 max-w-6xl mx-auto pb-20">
    <div class="flex items-end justify-between border-b border-blue-600/10 pb-10">
      <div>
        <h2 class="text-4xl font-black uppercase italic tracking-tighter">Command <span class="text-blue-600">Overview</span></h2>
        <p class="text-slate-500 font-bold uppercase tracking-[0.3em] text-[10px] mt-2">Active Strategic Intel</p>
      </div>
      <div class="bg-blue-600/10 px-4 py-2 rounded-lg border border-blue-600/20 text-[10px] font-black uppercase text-blue-500 tracking-widest animate-pulse">
        System Sync: Online
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <div v-for="stat in [
        { name: 'Total Operations', value: tournaments?.length || 0, icon: '🏆', color: 'blue' },
        { name: 'Active Squads', value: teams?.length || 0, icon: '🛡️', color: 'blue' },
        { name: 'Arsenal Diversity', value: games?.length || 0, icon: '⚔️', color: 'blue' },
        { name: 'Intel Signals', value: posts?.length || 0, icon: '📡', color: 'blue' }
      ]" :key="stat.name" class="bg-black/40 border border-blue-900/10 rounded-3xl p-8 group hover:border-blue-600/30 transition-all shadow-2xl relative overflow-hidden">
        <div class="absolute -top-10 -right-10 w-24 h-24 bg-blue-600/5 blur-3xl group-hover:bg-blue-600/10 transition-all"></div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
      <div class="bg-black/40 border border-slate-900 rounded-3xl p-10 space-y-8">
        <div class="flex justify-between items-center">
          <h3 class="text-xs font-black uppercase tracking-widest text-blue-500">Live Signals</h3>
        ...
        <div class="space-y-4">
          <div v-for="post in posts?.slice(0, 5)" :key="post.id" class="p-4 bg-slate-900/30 border border-red-900/5 rounded-2xl flex items-center justify-between group hover:border-red-900/20 transition-all">
            <div class="flex items-center gap-4">
              <div class="w-10 h-10 rounded-lg bg-blue-600 flex items-center justify-center font-black italic">
      ...
      <div class="bg-black/40 border border-slate-900 rounded-3xl p-10 space-y-8">
        <div class="flex justify-between items-center">
          <h3 class="text-xs font-black uppercase tracking-widest text-blue-500">Active Squads</h3>
        ...
        <div class="space-y-4">
          <div v-for="team in teams?.slice(0, 5)" :key="team.id" class="p-4 bg-slate-900/30 border border-red-900/5 rounded-2xl flex items-center justify-between group">
            ...
            <p class="text-[10px] font-bold text-blue-500/50 uppercase tracking-widest">{{ team.members_count }} Units</p>
          </div>
        </div>
      </div>
    </div>
