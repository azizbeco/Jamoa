...
<template>
  <div class="min-h-screen bg-slate-950 text-white font-sans">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <!-- Header Area -->
      <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 border-b border-red-900/20 pb-10 gap-6">
        <div class="space-y-2">
        ...
        <div class="flex items-center space-x-6 text-right">
          ...
          <button class="bg-red-600 hover:bg-red-700 text-white px-8 py-3 rounded font-black uppercase text-sm transition-all shadow-[0_0_20px_rgba(220,38,38,0.3)] skew-x-[-10deg]">
            <span class="inline-block skew-x-[10deg]">Create Squad</span>
          </button>
        </div>
      </div>
      ...
      <div v-else-if="error" class="bg-red-600/10 border border-red-600/30 p-10 rounded-2xl text-center">
        ...
        <p class="text-slate-500 max-w-md mx-auto mb-8">Unable to fetch teams from the API headquarters. Verify your connection.</p>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <!-- Empty State -->
        <div v-if="teams?.length === 0" class="col-span-full py-32 text-center border-2 border-dashed border-red-900/10 rounded-3xl">
          <div class="text-6xl mb-6 opacity-20">🛡️</div>
          <h2 class="text-2xl font-black uppercase text-slate-700">No Squads Formed Yet</h2>
          <p class="text-slate-500 mt-2 italic">Be the first to lead a new tactical unit.</p>
        </div>
      </div>
    </div>
  </div>
